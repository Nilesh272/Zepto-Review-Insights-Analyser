...
def build_gold_clusters(reviews, gold):
    """Group reviews into Cluster objects by their gold theme label (largest first)."""
    from pulse.models import Cluster

    groups: dict[int, list] = {}
    for r, g in zip(reviews, gold):
        groups.setdefault(g, []).append(r)

    clusters = []
    for theme, members in sorted(groups.items(), key=lambda kv: -len(kv[1])):
