...
def test_fetch_reviews_tool_offline(tmp_path, appstore_page1, play_entries):
    ...
    product = cfg.registry.get("groww")

    cache = RawCache(tmp_path / "cache")
    cache.put("app_store", "groww", "in_page1", appstore_page1)
    cache.put("play_store", "groww", "en_in_n400", play_entries)
    ...
    assert out["dropped"]["out_of_window"] == 1
