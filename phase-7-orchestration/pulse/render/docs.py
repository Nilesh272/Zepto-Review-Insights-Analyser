...
def build_docs_requests(
    ...
    *,
    ...
    heading = (
        f"{product_name} — AI Feedback Insights — {iso_week}"
        if insight_mode
        else f"{product_name} — Weekly Review Pulse — {iso_week}"
    )
    b.heading(heading, level=1, anchor=anchor)
    ...
    quote_label = cfg.insight_quote_label if insight_mode else cfg.theme_quote_label
    ...
    for theme in themes:
        ...
        if theme.who_this_helps:
            audiences = theme.who_this_helps
            if isinstance(audiences, str):
                audience_text = audiences.strip()
            else:
                audience_text = ", ".join(str(a).strip() for a in audiences if str(a).strip())
            if audience_text:
                b.paragraph("Who this helps: " + audience_text)
