...
class Budget:
    ...
    def would_exceed(self, *, tokens: int = 0, cost_usd: float = 0.0) -> bool:
        """True if adding this usage would breach either cap (no mutation).

        Lets tools (e.g. summarization) stop *before* spending and keep partial results,
        rather than raising mid-step (eval E4.8 / edge X4.9).
        """
        return (
            self.tokens + tokens > self.max_tokens
            or self.cost_usd + cost_usd > self.max_cost_usd
        )
