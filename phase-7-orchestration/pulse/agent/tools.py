...
Implemented per phase:
  ...
  docs_append_section  -> Phase 6 (Google Docs MCP)            [IMPLEMENTED]
  gmail_draft_or_send  -> Phase 6 (Gmail MCP)                  [IMPLEMENTED]
"""
...
def _cluster_reviews(ctx: RunContext) -> dict:
    ...
    cfg = ctx.settings.summarize
    balanced = select_balanced_clusters(
        clusters, top_n,
        min_critical=cfg.min_critical_themes,
        critical_max_rating=cfg.critical_max_rating,
    )
    ctx.bag["top_clusters"] = balanced
    return {
        "input_reviews": len(reviews),
        "clusters": len(clusters),
        "clustered_reviews": sum(c.size for c in clusters),
        "top_cluster_sizes": [c.size for c in balanced],
        "critical_clusters": sum(1 for c in balanced if c.avg_rating <= cfg.critical_max_rating),
    }


def _summarize_clusters(ctx: RunContext) -> dict:
    from pulse.reasoning.insights import extract_insights
    ...
    icfg = ctx.settings.insights

    if icfg.enabled and icfg.lenses:
        drafts = extract_insights(clean_reviews, icfg.lenses, icfg)
        halted = False
        ctx.bag["insight_mode"] = True
    else:
        drafts, halted = summarize_clusters(clusters, clean_reviews, ctx.settings, ctx.budget)
        ctx.bag["insight_mode"] = False
    ...
    return {
        ...
        "themes": len(drafts),
        "insight_mode": ctx.bag["insight_mode"],
...
def _render_report(ctx: RunContext) -> dict:
    ...
    insight_mode = ctx.bag.get("insight_mode", False)
...
def _docs_append_section(ctx: RunContext) -> dict:
    ...
    report = ctx.bag.get("rendered")
    if report is None:
        raise ValueError("docs_append_section requires a rendered report (run render_report first)")

    result = append_section(ctx, report)
    ctx.bag["doc_id"] = result["doc_id"]
    ctx.bag["section_anchor"] = result["section_anchor"]
    ctx.bag["heading_id"] = result["heading_id"]
    ctx.bag["deep_link"] = result["deep_link"]
    return result
...
def _gmail_draft_or_send(ctx: RunContext) -> dict:
    ...
    report = ctx.bag.get("rendered")
    if report is None:
        raise ValueError("gmail_draft_or_send requires a rendered report (run render_report first)")
    deep_link = ctx.bag.get("deep_link")
    if not deep_link:
        raise ValueError("gmail_draft_or_send requires a deep link (run docs_append_section first)")

    result = draft_or_send(ctx, report, deep_link=deep_link)
    ctx.bag["email_status"] = result["email_status"]
    ctx.bag["message_id"] = result.get("message_id")
    return result


def build_default_registry() -> ToolRegistry:
    """Register the agent's 6 skill tools + 2 MCP delivery tools."""
    reg = ToolRegistry()
