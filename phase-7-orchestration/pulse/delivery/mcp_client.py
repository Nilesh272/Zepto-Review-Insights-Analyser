...
def build_docs_client(settings) -> DocsMcpClient:
    ...
    if settings.mcp.transport == "local":
        from pulse.delivery.local_file_mcp import LocalFileDocsServer  # noqa: PLC0415

        return LocalFileDocsServer(settings.mcp.local_output_dir)
    from pulse.delivery.stdio_mcp import StdioDocsClient  # noqa: PLC0415

    return StdioDocsClient(settings.mcp.docs_server, settings.mcp.docs_tools)
...
def build_gmail_client(settings) -> GmailMcpClient:
    ...
    if settings.mcp.transport == "local":
        from pulse.delivery.local_file_mcp import LocalFileGmailServer  # noqa: PLC0415

        return LocalFileGmailServer(settings.mcp.local_output_dir)
    from pulse.delivery.stdio_mcp import StdioGmailClient  # noqa: PLC0415

    return StdioGmailClient(settings.mcp.gmail_server, settings.mcp.gmail_tools)
