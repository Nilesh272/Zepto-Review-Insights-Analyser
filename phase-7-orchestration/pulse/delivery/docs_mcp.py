...
def append_section(ctx, report, *, client=None) -> dict:
    ...
    # Let a backend provide its own link format (e.g. a local file://) and fall back to Google's.
    _link = getattr(client, "deep_link", deep_link)
    ...
    if existing and not ctx.force:
        ...
        return {
            ...
            "deep_link": _link(doc_id, hid), "appended": False, "replaced": False, "skipped": True,
        }
    ...
    return {
        "doc_id": doc_id, "section_anchor": anchor, "heading_id": hid,
        "deep_link": _link(doc_id, hid), "appended": True, "replaced": replaced, "skipped": False,
    }
