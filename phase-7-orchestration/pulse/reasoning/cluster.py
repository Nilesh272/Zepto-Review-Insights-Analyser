...
def _cluster_all(reviews, settings, embedder: Embedder | None = None) -> list[Cluster]:
    """Cluster one review set and return ranked clusters (empty if low-signal)."""
...
def cluster_reviews(reviews, settings, embedder: Embedder | None = None) -> list[Cluster]:
    ...
    When ``stratify_by_rating`` is enabled (default), 1-2★, 3★, and 4-5★ reviews are clustered
    separately so critical feedback is not absorbed into positive-majority groups.
    """
    cfg = settings.reasoning
    n = len(reviews)
    if n < cfg.min_reviews_for_clustering:
    ...
    if not cfg.stratify_by_rating:
        return _cluster_all(reviews, settings, embedder)
    ...
    for subset in buckets:
        if len(subset) >= cfg.min_cluster_size:
            merged.extend(_cluster_all(subset, settings, embedder))

    if not merged:
        return []

    merged.sort(key=lambda c: (-c.score, -c.size, c.review_ids[0]))
