...
class DeterministicSummarizer:
    ...
    def _quote_order(self, cluster, reviews: list):
        """Prefer low-star reviews for critical clusters, high-star for positive ones."""
        if cluster.avg_rating <= 2.5:
            return sorted(reviews, key=lambda r: (r.rating, -len(r.body_clean), r.review_id))
        if cluster.avg_rating >= 4.0:
            return sorted(reviews, key=lambda r: (-r.rating, -len(r.body_clean), r.review_id))
        return sorted(reviews, key=lambda r: (-len(r.body_clean), r.review_id))

    def _sentiment_prefix(self, avg_rating: float) -> str:
        if avg_rating <= 2.5:
            return "Critical — "
        if avg_rating >= 4.0:
            return "Positive — "
    ...
    def _summary_line(self, cluster, title_kw: list[str]) -> str:
        kw = ", ".join(title_kw) or "shared feedback"
        if cluster.avg_rating <= 2.5:
            lead = f"users report problems with {kw}"
        elif cluster.avg_rating >= 4.0:
            lead = f"users praise {kw}"
        else:
            lead = f"mixed feedback on {kw}"
        return (
            f"{cluster.size} reviews (avg rating {cluster.avg_rating:.1f}) — {lead}."
        )

    def summarize_cluster(self, cluster, reviews: list, cfg) -> tuple[ThemeDraft, int]:
        bodies = [r.body_clean for r in reviews]
        title_kw = keywords(bodies, k=2)
        title = self._sentiment_prefix(cluster.avg_rating) + (
            " & ".join(w.capitalize() for w in title_kw) or "General Feedback"
        )
...
class LLMSummarizer:
    ...
    def __init__(self, model: str, *, backend: str = "openai", client=None):
        self.model = model
    ...
    def _complete(self, system: str, user: str) -> tuple[str, int]:
        if self._client is None:
            from pulse.reasoning.providers import build_chat_client  # noqa: PLC0415

            self._client = build_chat_client(self.backend)
...
def build_summarizer(settings) -> ThemeSummarizer:
    from pulse.reasoning.providers import REMOTE_LLM_BACKENDS, default_model_for

    cfg = settings.summarize
    if cfg.backend in REMOTE_LLM_BACKENDS:
        model = default_model_for(cfg.backend, cfg.model)
        return LLMSummarizer(model, backend=cfg.backend)
