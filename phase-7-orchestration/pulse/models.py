...
class EmailDraft(BaseModel):
    """A teaser email with HTML + plain-text parts and a deep-link slot (Phase 5).

    The deep link is a placeholder until Phase 6 fills it after the Doc write
    (`deep_link_filled` stays False until then).
    """
    ...
    deep_link_filled: bool = False


class RenderedReport(BaseModel):
    """Output of `render_report`: Docs requests + email, ready for MCP delivery in Phase 6."""

    product_id: str
    iso_week: str
    section_anchor: str
    docs_requests: list[dict] = Field(default_factory=list)
    email: EmailDraft
