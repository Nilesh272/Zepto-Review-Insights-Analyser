...
def _run(agent, tmp_path, **kw):
    return agent.run(
        "zepto", "2026-W26", dry_run=True, offline=True, cache_dir=str(tmp_path / "cache"), **kw
    )
...
def test_dry_run_completes_and_records(tmp_path):
    ...
    assert summary.section_anchor == "pulse-zepto-2026-W26"

    rec = ledger.get("zepto", "2026-W26")
...
def test_bad_week_fails_fast(tmp_path):
    ...
    with pytest.raises(IsoWeekError):
        agent.run("zepto", "2026-W54", dry_run=True, offline=True)
    ledger.close()
